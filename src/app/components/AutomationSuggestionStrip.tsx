...
function getDrawerDescription(filterName: string): string {
  const map: Record<string, string> = {
...
function getTriggerCondition(filterName: string): string {
  const map: Record<string, string> = {
...
interface AutomationSuggestionStripProps {
  filterName: string;
  totalCount: number;
  handledCount: number;
...
export function AutomationSuggestionStrip({
  ...
  useEffect(() => {
    ...
    setEditableDescription(getDrawerDescription(filterName));
    setEditableTrigger(getTriggerCondition(filterName));
  ...
  return (
    <>
      {/* Suggestion strip */}
      <div
        className={`mx-3 transition-all duration-300 ease-in-out overflow-hidden ${
          isDismissing ? 'max-h-0 opacity-0 my-0 py-0' : 'max-h-56 opacity-100 my-2'
        }`}
      >
        <div className="relative bg-[#FAFAF9] border border-gray-200 rounded-lg px-4 py-3 shadow-[0_1px_4px_rgba(0,0,0,0.07)]">
          <button
          ...
          {!isConfirmed ? (
            <>
              <p className="text-sm text-gray-800 pr-5 leading-snug">
                You've handled <strong>{handledCount}</strong> of <strong>{totalCount}</strong>{' '}
                {label} conversations this week.
              </p>
              <p className="text-sm text-gray-500 mt-0.5 leading-snug">
                Want me to handle the next one automatically?
              </p>
              <div className="flex items-center gap-2 mt-3">
                <button
                  onClick={handleYesSetItUp}
                ...
                <button
                  ...
                  className="px-3 py-1.5 border border-gray-300 text-xs text-gray-600 rounded-md hover:bg-gray-50 hover:border-gray-400 transition-colors"
                >
          ...
          <div className="fixed top-0 right-0 h-full w-[400px] max-w-[90vw] z-50 bg-white border-l border-gray-200 shadow-xl flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full text-sm text-gray-800 border border-gray-200 rounded-md px-3 py-2 resize-none focus:outline-none focus:ring-1 focus:ring-[#23a455] focus:border-[#23a455] bg-white"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full text-sm text-gray-800 border border-gray-200 rounded-md px-3 py-2 focus:outline-none focus:ring-1 focus:ring-[#23a455] focus:border-[#23a455] bg-white"
                />
              </div>

              <p className="text-xs text-gray-400 leading-relaxed">
                This automation will only apply to conversations in the{' '}
                <span className="text-gray-600">{filterName}</span> signal. You can pause or edit
                it anytime from Insights.
              </p>
            </div>
            ...
            <div className="px-6 py-4 border-t border-gray-100 flex items-center gap-4">
              <button
                ...
                className="px-4 py-2 bg-[#23a455] text-white text-sm rounded-md hover:bg-[#1d8f47] transition-colors"
              >
                Save
              </button>
              <button
