...
interface CardAction { label: string; variant: ActionVariant; onClick?: () => void; }
interface PulseCardData {
  id: string; type: CardType; status: string; timestamp: string;
  headline: string; body: React.ReactNode; evidence?: React.ReactNode; actions: CardAction[];
}
interface Section { id: string; label: string; sublabel: string; cards: PulseCardData[]; }

function StatusPill({ status }: { status: string }) {
  const s: Record<string, string> = { NEW: 'bg-[#EDF1FA] text-[#173DA6]', ACTIVE: 'bg-[#ebf7f0] text-[#23a455]', PAUSED: 'bg-gray-100 text-gray-500' };
  return <span className={`inline-block px-1.5 py-0.5 rounded text-[10px] font-semibold uppercase tracking-wide ${s[status] ?? 'bg-gray-100 text-gray-500'}`}>{status}</span>;
}
...
function PulseCard({ type, status, timestamp, headline, body, evidence, actions }: PulseCardData) {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl px-6 py-5">
      ...
      {evidence && <div className="mt-3 bg-gray-50 border border-gray-100 rounded-lg px-4 py-3 text-sm text-gray-600">{evidence}</div>}
      <div className="flex items-center gap-5 mt-4">
        {actions.map(a => {
          if (a.variant === 'primary') return <button key={a.label} onClick={a.onClick} className="px-3 py-1.5 bg-[#23a455] text-white text-xs rounded-md hover:bg-[#1d8f47]">{a.label}</button>;
          if (a.variant === 'secondary') return <button key={a.label} onClick={a.onClick} className="text-xs text-gray-700 underline underline-offset-2 hover:text-gray-900">{a.label}</button>;
          return <button key={a.label} onClick={() => { a.onClick?.(); setDismissed(true); }} className="text-xs text-gray-400 hover:text-gray-600">{a.label}</button>;
        })}
      </div>
    </div>
  );
...
function PulseSection({ label, sublabel, cards, defaultOpen = false }: Section & { defaultOpen?: boolean }) {
  ...
  return (
    <section className="border border-gray-200 rounded-xl bg-white overflow-hidden">
      <button onClick={() => setIsOpen(p => !p)} className="w-full flex items-center justify-between px-5 py-4 hover:bg-gray-50 transition-colors">
        <div className="flex items-center gap-3">
          <span className="text-sm font-medium text-gray-800">{label}</span>
          {cards.length > 0 && <span className="text-[10px] font-semibold bg-gray-100 text-gray-500 px-1.5 py-0.5 rounded-full">{cards.length}</span>}
      ...
      <div className={`transition-all duration-300 overflow-hidden ${isOpen ? 'max-h-[2000px] opacity-100' : 'max-h-0 opacity-0'}`}>
        <div className="px-5 pb-5 border-t border-gray-100">
          {cards.length > 0
            ? <div className="space-y-3 pt-4">{cards.map(c => <PulseCard key={c.id} {...c} />)}</div>
...
const USE_CASES = [
  { category: 'Customer insights', items: [
            ...
            <div key={group.category}>
              ...
              <div className="grid gap-2">
                {group.items.map(item => (
                  <button key={item.prompt} onClick={onClose}
...
function VibeOverlay({ query, onChange, onClose, onOpenUseCases }: { query: string; onChange: (v: string) => void; onClose: () => void; onOpenUseCases: () => void; }) {
    ...
    <div className="fixed inset-0 z-40 flex flex-col items-center justify-center px-4">
      ...
      <div className="relative w-full max-w-xl space-y-4">
        ...
        <div className="relative">
          ...
          <button onClick={() => { if (query.trim()) { onChange(''); onClose(); } }}
            className="absolute right-3.5 top-1/2 -translate-y-1/2 w-7 h-7 flex items-center justify-center rounded-full bg-[#23a455] text-white hover:bg-[#1d8f47]">
            <Send className="w-3.5 h-3.5" />
          </button>
        </div>
        <div className="bg-white rounded-xl border border-gray-100 shadow-lg overflow-hidden">
          {QUICK_OPTIONS.map((opt, i) => (
            <button key={opt} onClick={() => { onChange(opt); inputRef.current?.focus(); }}
              className={`w-full flex items-center justify-between px-5 py-3 text-sm text-gray-700 hover:bg-gray-50 text-left ${i < QUICK_OPTIONS.length - 1 ? 'border-b border-gray-100' : ''}`}>
              <span>{opt}</span>
              <ChevronRight className="w-3.5 h-3.5 text-gray-300 shrink-0" />
            </button>
          ))}
        </div>
        <button onClick={e => { e.stopPropagation(); onOpenUseCases(); }}
          className="w-full flex items-center justify-between bg-white rounded-xl border border-gray-100 shadow-lg px-5 py-4 hover:bg-gray-50 group">
          <div className="flex items-center gap-3">
...
export function PulsePage() {
  ...
  const sections: Section[] = [
    {
      id: 'patterns', label: 'Patterns', sublabel: 'things the system noticed on its own',
      cards: [
        { id: 'p1', type: 'PATTERN', status: 'NEW', timestamp: 'discovered yesterday',
          ...
          evidence: <span>Saturday avg: <strong>4h 12m</strong> · Weekday avg: <strong>58m</strong> · Saturday volume: <strong>23 conversations</strong></span>,
          actions: [{ label: 'Set up a Saturday watcher', variant: 'primary' }, { label: 'Dismiss', variant: 'tertiary' }] },
        { id: 'p2', type: 'PATTERN', status: 'NEW', timestamp: 'discovered 3 days ago',
          ...
          actions: [{ label: 'Investigate', variant: 'primary' }, { label: 'Dismiss', variant: 'tertiary' }] },
      ],
    },
    {
      id: 'recurring', label: 'Recurring actions', sublabel: 'automations the owner has set up',
      cards: [
        { id: 'r1', type: 'RECURRING', status: 'ACTIVE', timestamp: 'active since 3 May',
          headline: 'Drop-off follow-up.',
          body: 'Automatically sends a follow-up to contacts who go quiet for 3+ days after reaching negotiating stage.',
          evidence: <span><strong>12 contacts</strong> followed up · <strong>3 re-engaged</strong> this week.</span>,
          actions: [{ label: 'Edit', variant: 'secondary' }, { label: 'Pause', variant: 'tertiary' }] },
      ],
    },
    {
      id: 'digest', label: 'Weekly digest', sublabel: 'the Monday summary',
      cards: [
        { id: 'd1', type: 'DIGEST', status: 'MON 9:00 AM', timestamp: 'this week',
          ...
          actions: [{ label: 'Open full digest', variant: 'secondary' }, { label: 'Dismiss', variant: 'tertiary' }] },
      ],
    },
  ];
      ...
      <div className="px-8 pb-6 shrink-0">
        <div className="relative">
          <input type="text" value={query} onChange={e => setQuery(e.target.value)}
            onFocus={() => setIsExpanded(true)} onClick={() => setIsExpanded(true)} readOnly={!isExpanded}
            placeholder="Ask anything about your customers…"
            className="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 pr-11 text-sm text-gray-800 placeholder:text-gray-400 focus:outline-none cursor-pointer hover:border-gray-300" />
          <button onClick={() => setIsExpanded(true)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-[#23a455]">
            <Send className="w-4 h-4" />
          </button>
        </div>
      </div>
      <div className="flex-1 overflow-y-auto px-8 pb-10 space-y-2">
        {sections.map((s, i) => <PulseSection key={s.id} {...s} defaultOpen={i === 0} />)}
      </div>
      {isExpanded && <VibeOverlay query={query} onChange={setQuery} onClose={() => setIsExpanded(false)} onOpenUseCases={() => setShowUseCases(true)} />}
      {showUseCases && <UseCasesModal onClose={() => setShowUseCases(false)} />}
    </div>
  );
}
